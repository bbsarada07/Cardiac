...
class RFAnomalyClassifier:
    ...
    def __init__(self):
        ...
        self.labels = ["Normal", "Early Warning", "High Risk", "Critical"]
    ...
    def classify(self, hr, sdnn, stability, current_time=None):
        if not self.is_trained:
            return "Calibrating..."
        ...
        if t - self.last_classification_time >= 30:
            probs = self.classifier.predict_proba([[np.float64(hr), np.float64(sdnn), np.float64(stability)]])[0]
